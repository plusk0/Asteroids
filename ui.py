...
class UIManager:
    ...
    def draw_glow_rect(self, screen, color, rect, radius=15, glow_size=5):
        ...
        for i in range(glow_size, 0, -1):
            ...
            pygame.draw.rect(
                s, glow_color, s.get_rect(), width=0, border_radius=radius + i
            )
            screen.blit(s, glow_rect.topleft)
        pygame.draw.rect(screen, color, rect, width=0, border_radius=radius)

    def draw_button(
        self, screen, text, rect, action=None, selected=False, disabled=False
    ):
    ...
    def draw_card(
        ...
        if icon:
            ...
            pygame.draw.circle(
                screen, colors["border"], icon_rect.center, icon_rect.width // 2
            )
            # Draw icon symbol
            if icon == "multishot":
                ...
                for i in range(3):
                    ...
                    pygame.draw.circle(
                        screen, Colors.TEXTBRIGHT, icon_rect.center + offset, 4
                    )
            elif icon == "shield":
                pygame.draw.circle(
                    screen,
                ...
                for i in range(3):
                    pygame.draw.circle(
                        screen,
                        Colors.TEXTBRIGHT,
                        icon_rect.center + pygame.Vector2(-10 + i * 10, -5 + i * 5),
                        3,
                    )
            elif icon == "bigger":
                ...
                # Draw multiple small circles for wingmen
                for i in range(2):
                    angle = i * 180
                    pos = icon_rect.center + pygame.Vector2(0, -6).rotate(angle)
                    pygame.draw.circle(screen, Colors.TEXTBRIGHT, pos, 3)
                # Draw player in center
                pygame.draw.circle(screen, Colors.TEXTBRIGHT, icon_rect.center, 2)
            elif icon == "magnetizer":
                # Draw repulsion waves - greyish color
                pygame.draw.circle(screen, (180, 180, 180), icon_rect.center, 8, 1)
                pygame.draw.circle(screen, (180, 180, 180), icon_rect.center, 12, 1)
                # Draw arrows pointing outward
                for i in range(4):
                    angle = i * 90
                    start_pos = icon_rect.center + pygame.Vector2(0, -5).rotate(angle)
                    end_pos = icon_rect.center + pygame.Vector2(0, -10).rotate(angle)
                    pygame.draw.line(screen, (180, 180, 180), start_pos, end_pos, 2)
            elif icon == "shockwave":
                # Draw a shockwave pulse
                pygame.draw.line(
                    screen,
                    Colors.TEXTBRIGHT,
                    icon_rect.center + pygame.Vector2(-15, 0),
                    icon_rect.center + pygame.Vector2(15, 0),
                    3,
                )
                # Add wave effect
                pygame.draw.circle(screen, Colors.TEXTBRIGHT, icon_rect.center, 10, 1)
                pygame.draw.circle(screen, Colors.TEXTBRIGHT, icon_rect.center, 6, 1)
            elif icon == "formations":
                # Draw 3 small circles in a triangle for formations
                for i in range(3):
                    angle = i * 120
                    pos = icon_rect.center + pygame.Vector2(0, -8).rotate(angle)
                    color = [(255, 200, 100), (100, 200, 255), (200, 100, 200)][i]
                    pygame.draw.circle(screen, color, pos, 4)
            elif icon == "maneuvers":
                # Draw wingmen with +/- buttons
                # Draw wingmen
                for i in range(2):
                    angle = i * 180
                    pos = icon_rect.center + pygame.Vector2(0, -6).rotate(angle)
                ...
                # Draw player in center
                pygame.draw.circle(screen, Colors.TEXTBRIGHT, icon_rect.center, 2)
                # Draw small + and - symbols
                pygame.draw.line(
                    screen,
                    Colors.TEXTBRIGHT,
                    icon_rect.center + pygame.Vector2(-8, 5),
                    icon_rect.center + pygame.Vector2(-6, 5),
                    1,
                )
                pygame.draw.line(
                    screen,
                    Colors.TEXTBRIGHT,
                    icon_rect.center + pygame.Vector2(-7, 4),
                    icon_rect.center + pygame.Vector2(-7, 6),
                    1,
                )
                pygame.draw.line(
                    screen,
    ...
    def draw_health_bar(self, screen, current, max_health, position, spacing=5):
        ...
        for i in range(max_health):
            ...
            # Draw heart shape
            points = [(x, y - 10), (x - 8, y), (x, y + 8), (x + 8, y)]
    ...
    def draw_exp_bar(
        self, screen, current_exp, exp_for_next_level, position, size=(200, 20)
    ):
    ...
    def draw_main_menu(self, screen, menu_items, selected_item=None):
        ...
        self.draw_button(
            screen, "SETTINGS", menu_items["settings"], selected=settings_selected
        )
        self.draw_button(screen, "QUIT", menu_items["quit"], selected=quit_selected)

        # Draw version
        version_text = self.font_tiny.render(
            menu_items["version_text"], True, Colors.TEXT_MUTED
        )
    ...
    def draw_difficulty_menu(self, screen, menu_data, selected_index=None):
        ...
        for i, (card_rect, diff) in enumerate(menu_data["cards"]):
            ...
            if is_selected or mouse_over:
                self.draw_glow_rect(
                    screen, diff["color"], card_rect, radius=15, glow_size=5
                )
            else:
                pygame.draw.rect(
                    screen, diff["color"], card_rect, width=3, border_radius=15
                )
            ...
            # Draw hotkey
            hotkey_surf = self.font_small.render(
                f"Press {i + 1}", True, Colors.TEXT_MUTED
            )
            hotkey_rect = hotkey_surf.get_rect(
                center=(card_rect.centerx, card_rect.bottom - 20)
            )
            screen.blit(hotkey_surf, hotkey_rect)

        # Draw back button
        self.draw_button(
            screen, "BACK", menu_data["back"], selected=(selected_index == "back")
        )

        return screen

    def create_upgrade_menu(self, screen, upgrade_options, player):
        """Create upgrade selection menu with vertical layout"""
        center_x = self.screen_width // 2
        ...
        card_height = 100
        card_spacing = 120
        start_y = center_y - (len(upgrade_options) - 1) * card_spacing // 2
        ...
        if (
            ...
            and hasattr(player.weapon_manager, "wingmen")
        ):
            ...
            if wingmen.formations_unlocked:
                ...
                formation_card_height = 60  # Shorter height
                formation_spacing_h = 160  # Horizontal spacing between formations

                # Center the formations
                formations_start_x = (
                    center_x - (3 * formation_card_width + 2 * formation_spacing_h) // 2
                )

                for i, formation_name in enumerate(constants.WINGMEN_FORMATIONS):
                    formation_rect = pygame.Rect(
                        formations_start_x
                        + i * (formation_card_width + formation_spacing_h),
                        formation_spacing,
                        formation_card_width,
    ...
    def get_upgrade_description(self, upgrade):
        ...
        descriptions = {
            ...
            "Wingmen": "Fighter escorts that attack asteroids",
            "Magnetizer": "Pushes asteroids away from you",
    ...
    def draw_upgrade_menu(self, screen, menu_data, player):
        """Draw upgrade selection menu with vertical layout"""
        # Dim background
        overlay = pygame.Surface(
            (self.screen_width, self.screen_height), pygame.SRCALPHA
        )
        ...
        # Draw title - moved up for vertical layout
        title = self.font_medium.render(
            "LEVEL UP! CHOOSE AN UPGRADE", True, Colors.ACCENT
        )
        title_rect = title.get_rect(
        ...
        # Draw level info (if player is available)
        if player is not None:
            level_info = self.font_small.render(
                f"Level {player.level}", True, Colors.TEXT_MUTED
            )
        ...
        for i, (card_rect, upgrade, icon, description, rarity) in enumerate(
            menu_data["cards"]
        ):
            hotkey = self.font_small.render(f"{i + 1}", True, Colors.TEXTBRIGHT)
            hotkey_rect = hotkey.get_rect(
                bottomright=(card_rect.right - 10, card_rect.bottom - 10)
            )
            # Draw hotkey background
            pygame.draw.rect(
                screen,
                Colors.SURFACE,
                hotkey_rect.inflate(10, 5),
                width=0,
                border_radius=5,
            )
            pygame.draw.rect(
        ...
        if "formations" in menu_data and menu_data["formations"]:
            for formation_data in menu_data["formations"]:
                ...
                name_font = self.font_regular
                name_surf = name_font.render(formation_name, True, Colors.TEXTBRIGHT)
    ...
    def draw_hud(self, screen, player):
        ...
        # Score in top-right
        score_text = self.font_regular.render(
            f"{player.score}", True, Colors.TEXTBRIGHT
        )
        ...
        # Level in bottom-right corner (moved from top-right)
        level_text = self.font_regular.render(
            f"LEVEL: {player.level}", True, Colors.ACCENT
        )
        ...
        # EXP Bar - positioned above level in bottom-right
        exp_needed = 5 * (
            player.level**1.5
        )  # Match the level up condition in player.py
        if exp_needed > 0:
            exp_bar_rect = pygame.Rect(
                self.screen_width // 2 - 150, self.screen_height - 40, 300, 20
            )
            ...
            for i, weapon in enumerate(player.weapon_manager.weapons):
                if weapon.level > 0:
                    ...
                    icon_rect = pygame.Rect(
                        weapon_x + i * (icon_size + 15), weapon_y, icon_size, icon_size
                    )

                    weapon_name = weapon.__class__.__name__.lower()
                    if "rotator" in weapon_name:
                        pygame.draw.circle(
                            screen, Colors.INFO, icon_rect.center, icon_size // 2, 2
                        )
                        for j in range(3):
                            angle = j * 120
                            pos = icon_rect.center + pygame.Vector2(
                                0, -icon_size // 2
                            ).rotate(angle)
                            pygame.draw.circle(screen, Colors.INFO, pos, 4)
                    elif "emp" in weapon_name:
                        pygame.draw.circle(
                            screen, Colors.DANGER, icon_rect.center, icon_size // 2
                        )
                        pygame.draw.circle(
                            screen, Colors.DANGER, icon_rect.center, icon_size // 3, 1
                        )
                    ...
                    if weapon.level > 1:
                        level_indicator = self.font_tiny.render(
                            str(weapon.level), True, Colors.TEXTBRIGHT
                        )
                        level_rect = level_indicator.get_rect(
                            bottomright=icon_rect.bottomright
                        )
    ...
    def draw_game_over_menu(self, screen, menu_data):
        ...
        score = self.font_medium.render(
            f"Final Score: {player.score}", True, Colors.TEXTBRIGHT
        )
        score_rect = score.get_rect(center=(panel.centerx, stats_y))
        screen.blit(score, score_rect)

        level = self.font_medium.render(
            f"Level Reached: {player.level}", True, Colors.TEXTBRIGHT
        )
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    for name in button_names:
                        if name in menu_data and menu_data[name].collidepoint(
                            mouse_pos
                        ):
                            return name
                    return None
